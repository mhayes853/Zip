...
class ZipTests: XCTestCase {
  ...
  private func url(forResource resource: String, withExtension ext: String? = nil) -> URL? {
      ...
      let resourcePath = testDirPath.appendingPathComponent("Resources")
  ...
  func testQuickUnzipProgress() throws {
    let filePath = url(forResource: "bb8", withExtension: "zip")!
    let destinationURL = try Zip.quickUnzipFile(
      filePath,
  ...
  func testUnzipWithUnsupportedPermissions() throws {
    ...
    let permission644 = unzipDestination.appendingPathComponent("unsupported_permission")
      .appendingPathExtension("txt")
    let foundPermissions =
      try FileManager.default.attributesOfItem(atPath: permission644.path)[.posixPermissions]
      as? Int
    #if os(Linux)
  ...
  func testUnzipPermissions() throws {
    ...
    let fileManager = FileManager.default
    let permission777 = unzipDestination.appendingPathComponent("permission_777")
      .appendingPathExtension("txt")
    let permission600 = unzipDestination.appendingPathComponent("permission_600")
      .appendingPathExtension("txt")
    let permission604 = unzipDestination.appendingPathComponent("permission_604")
      .appendingPathExtension("txt")
  ...
  func testZipHeaderWithPasswordAESInfoWithNonConformingExtraData() throws {
    ...
    let header = try Zip.unzipHeader(zipFilePath)
    XCTAssertEqual(header.compressionMethod, .aes)
  }
